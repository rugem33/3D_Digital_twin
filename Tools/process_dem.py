...
def step2_generate_tiles(wgs84_tif: Path, max_zoom: int, nodata=NODATA_VAL,
    ...
    if shutil.which("ctb-tile"):
        # ctb-tile이 PATH에 있을 때 직접 실행 (Docker 컨테이너 내부 등)
        run(
            f'ctb-tile -f Mesh '
            f'-s {max_zoom} -e 0 '
            f'-o "{TILES_DIR}" "{wgs84_tif}"',
            "ctb-tile 직접 실행"
        )
    else:
        # Docker를 통한 실행 (호스트 환경)
...
def main():
    ...
    ctb_ok    = check_tool("ctb-tile")
    docker_ok = check_tool("docker")
    print(f"  GDAL    : {'OK' if gdal_ok   else 'MISSING (--info 참조)'}")
    print(f"  ctb-tile: {'OK (직접)' if ctb_ok else '없음'}")
    print(f"  Docker  : {'OK' if docker_ok else 'MISSING (--info 참조)'}")

    if not ctb_ok and not docker_ok:
        print("\nctb-tile 또는 Docker가 필요합니다. --info 옵션으로 설치 안내를 확인하세요.")
        sys.exit(1)
    ...
    # ctb-tile Docker 이미지 확인 (ctb-tile 직접 없을 때만)
    if not ctb_ok and not check_docker_image():
